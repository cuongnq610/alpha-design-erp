(() => {
  ...
  const viewMeta = {
    ...
    clients:['Khách hàng','Danh mục và quản lý khách hàng','◎'],
  ...
  function renderClients(){
    const rows=[...db.clients].sort((a,b)=>String(a.code||'').localeCompare(String(b.code||'')));
    const active=rows.filter(x=>String(x.status||'').toLowerCase()==='active').length;
    const leads=rows.filter(x=>String(x.status||'').toLowerCase()==='lead').length;
    return `<div class="grid kpi-grid">${kpi('Tổng khách hàng',fmtNum(rows.length,0),'Toàn bộ danh mục',true,{icon:'◎',color:'blue'})}${kpi('Đang hoạt động',fmtNum(active,0),'Trạng thái Active',true,{icon:'✓',color:'green'})}${kpi('Tiềm năng (Lead)',fmtNum(leads,0),'Chưa chuyển đổi',true,{icon:'◔',color:'orange'})}</div>
      <div class="card table-card section"><div class="section-header card-pad" style="margin-bottom:0"><div><h2>Danh mục khách hàng</h2><p>Tạo, cập nhật và xóa khách hàng.</p></div><button class="section-link" data-secondary-add="clients">+ Khách hàng</button></div><div class="table-tools"><input class="search-input" data-table-search="clients" aria-label="Tìm kiếm khách hàng" placeholder="Tìm mã, tên, người liên hệ, điện thoại..."></div><div id="clientsTable">${clientsTable(rows)}</div></div>`;
  }
  ...
  function bindViewEvents(){
    ...
    const searchers={
      ...
      clients:['[data-table-search="clients"]',null,()=>[...db.clients].sort((a,b)=>String(a.code||'').localeCompare(String(b.code||''))),'clientsTable',clientsTable,(x,q)=>`${x.code} ${x.name} ${x.contact} ${x.phone} ${x.taxCode}`.toLowerCase().includes(q)]
    };
...
Nhập vào danh sách phiên bản?`))return;
  ...
  const forms = {
    ...
    accounts:{title:'Tài khoản kế toán',help:'Tạo tài khoản chi tiết phục vụ quản trị; tài khoản 242/244 có thể cấu hình phân loại ngắn hạn hoặc dài hạn.',prefix:'acc',html:x=>`${field('code','Số hiệu tài khoản','text',x.code||'',[],false,'required maxlength="32" pattern="[0-9A-Za-z._-]+" title="Chỉ gồm chữ, số và các ký tự . _ - (không dấu cách), tối đa 32 ký tự"')}${field('name','Tên tài khoản','text',x.name||'',[],true,'required')}${field('type','Nhóm tài khoản','select',x.type||'Asset',['Asset','Liability','Equity','Revenue','Expense'].map(v=>({value:v,label:v})))}${field('normalSide','Tính chất số dư','select',x.normalSide||'Debit',[{value:'Debit',label:'Dư Nợ'},{value:'Credit',label:'Dư Có'}])}${field('reportClass','Phân loại B01a','select',x.reportClass||'',[{value:'',label:'Mặc định theo số hiệu tài khoản'},{value:'current_other_asset',label:'Tài sản ngắn hạn khác'},{value:'noncurrent_other_asset',label:'Tài sản dài hạn khác'}])}${field('active','Trạng thái','select',String(x.active??true),[{value:'true',label:'Active'},{value:'false',label:'Inactive'}])}`,parse:fd=>({code:String(fd.get('code')||'').trim(),name:fd.get('name'),type:fd.get('type'),normalSide:fd.get('normalSide'),reportClass:fd.get('reportClass')||'',active:fd.get('active')==='true'}),validate:(data,id)=>{const code=String(data.code||'').trim();if(!code)return 'Vui lòng nhập số hiệu tài khoản.';if(code.length>32)return 'Số hiệu tài khoản tối đa 32 ký tự.';if(!/^[0-9A-Za-z._-]+$/.test(code))return 'Số hiệu tài khoản chỉ gồm chữ, số và các ký tự . _ - (không dấu cách).';const dup=db.accounts.find(a=>a.code===code&&a.id!==id);return dup?'Số hiệu tài khoản đã tồn tại.':'';}},
