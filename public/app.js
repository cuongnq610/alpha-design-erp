(() => {
  ...
  const Money = window.AlphaMoneyInput;
  ...
  const MONEY_DECIMALS={costRate:2,hourlyRate:2,billingRate:2,unitPrice:2};
  function field(name,label,type='text',value='',options=[],full=false,attrs=''){
    ...
    if(type==='money'){
      const decimals=MONEY_DECIMALS[name]||0;
      const display=Money?Money.numericToDisplay(value,{decimals}):esc(value);
      return `<div class="${cls} field-money"><label for="${controlId}">${label}</label><input id="${controlId}" name="${name}" type="text" inputmode="decimal" autocomplete="off" data-money data-money-decimals="${decimals}" value="${esc(display)}" ${attrs}/></div>`;
    }
  ...
  function renderSettings(){
    ...
    return `<div class="grid two-col"><div class="card settings-card"><h3>Cấu hình quản trị</h3><p>Các tham số ảnh hưởng trực tiếp đến chỉ số tài chính và chi phí.</p><form id="settingsForm" class="form-grid">
      ...
      ${field('pitWithholdingThreshold','Ngưỡng TNCN hiện hành / lần (VND)','money',s.pitWithholdingThreshold??5000000,[],false,'min="0" step="100000"')}
      ${field('pitWithholdingThresholdPrevious','Ngưỡng TNCN trước ngày hiệu lực (VND)','money',s.pitWithholdingThresholdPrevious??2000000,[],false,'min="0" step="100000"')}
      ...
      ${field('latePaymentDailyRate','Tiền chậm nộp tham chiếu (%/ngày)','number',s.latePaymentDailyRate||0.03,[],false,'min="0" max="1" step="0.001"')}${field('maxContractValue','Ngưỡng cảnh báo giá trị hợp đồng (VND)','money',s.maxContractValue||1000000000000,[],false,'min="1000000" step="1000000"')}${field('fixedAssetThreshold','Ngưỡng ghi nhận TSCĐ (VND)','money',s.fixedAssetThreshold||30000000,[],false,'min="0" step="1000000"')}${field('toolMaxAllocationMonths','Phân bổ CCDC tối đa (tháng)','number',s.toolMaxAllocationMonths||36,[],false,'min="1" max="120" step="1"')}${field('procurementPolicyVersion','Quy chế mua sắm & tài sản','text',s.procurementPolicyVersion||'ALPHA-PROC-ASSET-2026.01',[],true,'readonly')}${field('forecastHorizonMonths','Kỳ dự báo tài chính (tháng)','number',s.forecastHorizonMonths||12,[],false,'min="3" max="36" step="1"')}${field('minimumCashBuffer','Ngưỡng tiền an toàn tối thiểu (VND)','money',s.minimumCashBuffer||150000000,[],false,'min="0" step="10000000"')}${field('financialAnalyticsPolicyVersion','Quy chế phân tích & dự báo','text',s.financialAnalyticsPolicyVersion||'ALPHA-FINANCE-ANALYTICS-2026.01',[],true,'readonly')}
...
Nhập vào danh sách phiên bản?`))return;
    ...
    if(sf)sf.onsubmit=(event)=>{
      ...
      if(!ensureWritable())return;
      const fd=moneyFormData(sf),previousRegime=db.settings.accountingRegime||'',previousCitRate=Number(db.settings.corporateTaxRate||0),previousCitEffectiveDate=String(db.settings.corporateTaxRateEffectiveDate||'');
  ...
  function formNumber(name){const el=modalForm.elements.namedItem(name);if(!el)return 0;const raw=Money&&el.matches?.('[data-money]')?Money.parseMoneyInput(el.value):el.value;return Number(raw||0);}
  ...
  function setFormValue(name,value,{onlyBlank=false}={}){
    ...
    if(Money&&control.matches?.('[data-money]'))control.value=Money.numericToDisplay(value,{decimals:Number(control.dataset.moneyDecimals||0)});
    else control.value=value??'';
    control.dispatchEvent(new Event('change',{bubbles:true}));
  }
  // Build FormData from a form, converting every money input's grouped display
  // value ("1.234.567,89") back to a plain JS-numeric string so downstream
  // Number(fd.get(...)) parsing works unchanged.
  function moneyFormData(formEl){
    const fd=new FormData(formEl);
    if(Money)formEl.querySelectorAll('[data-money]').forEach(el=>{if(el.name)fd.set(el.name,Money.parseMoneyInput(el.value));});
    return fd;
  }
  ...
  if(Money)modalForm.addEventListener('input',e=>{
    const el=e.target;
    if(!el?.matches?.('[data-money]'))return;
    const decimals=Number(el.dataset.moneyDecimals||0);
    const {value,caret}=Money.formatMoneyCaret(el.value,el.selectionStart,{decimals});
    el.value=value;
